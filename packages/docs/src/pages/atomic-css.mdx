...
Atomic CSS reduces the total amount defined rules by creating a single rule for every declaration —
enabling large re-use.

Take a standard rule:
...
As your product grows rules are created less often.

## Overriding through composition

Compiled [can't create CSS at runtime](/limitations),
because of this [composition of styles](/composition) may seem out of reach.
Naively depending on the order of rendered components you'd get different results.
