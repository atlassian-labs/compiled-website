...
Sometimes you'll want to keep using the same styled component but want to change the rendered markup to something else.
Maybe you want to change the heading level,
or the markup rendered in a specific context doesn't make sense as the original one anymore.
The `as` prop allows you to change it at runtime.
