...
Everything you need to use the component is included along side it.
...
React.forwardRef(({ as: C = 'span', ...props }, ref) => (
  <>
    ...
    <C
      {...props}
      ref={ref}
      className={'css-zd46j1' + (props.className ? ' ' + props.className : '')}
    />
  </>
));
```
...
and without needing them to setup anything they can use it as is.
This is powerful,
